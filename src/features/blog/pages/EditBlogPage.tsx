...
import { updateBlog, getBlogById } from '../components/api/api';
  ...
  useEffect(() => {
    const loadBlogData = async () => {
      ...
      try {
        ...
        console.log('EditBlogPage - API Response:', response);
        ...
        if (response.success && response.data) {
          ...
          if (Array.isArray(blog.category)) {
            ...
            categories = blog.category.split(',').map(cat => cat.trim());
          }
          setSelectedCategories(categories);
          setValue('category', categories.join(','));
          
          // Handle keywords - clean up malformed JSON strings
          let blogKeywords = [];
          if (Array.isArray(blog.keywords)) {
            blogKeywords = blog.keywords.map(keyword => {
              // Clean up malformed JSON strings like "[\"business\""
            ...
            blogKeywords = blog.keywords.split(',').map(k => k.trim()).filter(k => k);
          }
          setKeywords(blogKeywords);
